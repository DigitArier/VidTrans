...
# Transkriberen
def transcribe_audio_with_timestamps_safe(audio_file, transcription_file, batch_save_interval=10):
    """
    ...
    try:
        ...
        with gpu_context():
            ...
            vad_params = {
                "threshold": 0.45,               # Niedriger Schwellwert für empfindlichere Spracherkennung
                "min_speech_duration_ms": 250,  # Minimale Sprachdauer in Millisekunden
                #"max_speech_duration_s": 30,    # Maximale Sprachdauer in Sekunden
                "min_silence_duration_ms": 1000, # Minimale Stille-Dauer zwischen Segmenten
                "speech_pad_ms": 200            # Polsterzeit vor und nach Sprachsegmenten
            }
            
            # Der `transcribe`-Aufruf gibt einen Generator zurück, der sofort verarbeitet wird.
            segments_generator, info = pipeline.transcribe(
                ...
                hallucination_silence_threshold=1.0,  # Schwellenwert für Halluzinationen
        ...
        logger.info(f"Transkription abgeschlossen in {transcript_end_time:.2f} Sekunden -> {(transcript_end_time / 60):.2f} Minuten")
...
def translate_batch_madlad400(
    ...
    """
    Korrigierte Batch-Übersetzung mit robuster Eingabevalidierung für T5TokenizerFast.
    """
    if not texts:
        logger.warning("Leere Textliste zur Übersetzung erhalten")
        return []
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Übersetze Batches", unit="batch"):
        ...
        try:
            ...
            # KORREKTUR: Kein Target-Prefix bei MADLAD400!
...
def text_to_speech_with_voice_cloning(
    ...
    und korrekte, kumulative Zeitberechnung die exakte Gesamtlänge und das Timing wiederherstellt.
    Enthält eine robuste Validierung beim Lesen der Manifest-Datei, um IndexErrors zu verhindern.
    """
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()
    ...
    if os.path.exists(output_path):
        if not ask_overwrite(output_path):
            logger.info(f"Finale TTS-Audiodatei bereits vorhanden, überspringe: {output_path}")
            return
        else:
    ...
    try:
        ...
        # Stufe 1: Ausfallsichere TTS-Synthese
        processed_segments = {}
        if os.path.exists(TTS_PROGRESS_MANIFEST):
            with open(TTS_PROGRESS_MANIFEST, mode='r', encoding='utf-8') as f:
                ...
                try:
                    ...
                    for row in reader:
                        if len(row) >= 4:
                            processed_segments[(row[0], row[2])] = row[3]
        ...
        with open(translation_file, mode="r", encoding="utf-8", errors="replace") as f:
            ...
            for row in reader:
                if len(row) >= 3:
                    all_input_segments.append({'startzeit': row[0], 'endzeit': row[1], 'text': row[2].strip()})
        
        segments_to_process = [seg for seg in all_input_segments if (seg['startzeit'], seg['text']) not in processed_segments]
        
        processed_count = len(processed_segments)

        if not segments_to_process:
            logger.info("Alle Segmente wurden bereits synthetisiert. Gehe direkt zur Assemblierung.")
        else:
            logger.info(f"{len(segments_to_process)} von {len(all_input_segments)} Segmenten werden synthetisiert.")
            
            with gpu_context():
                print("Lade Basis-TTS-Modell...")
                xtts_model = load_xtts_v2()
                
                print("Generiere Stimm-Bedingungen...")
                with torch.inference_mode():
                    gpt_cond_latent, speaker_embedding = xtts_model.get_conditioning_latents(
                        audio_path=[sample_path_1, sample_path_2, sample_path_3]
                    )

                # KORREKTUR: DeepSpeed-Engine wird initialisiert
                print("Initialisiere DeepSpeed Inference für TTS...")
                ds_engine = deepspeed.init_inference(
                    model=xtts_model, tensor_parallel={"tp_size": 1}, 
                ...
                # Manuelle Batch-Verarbeitung
                batch_iterator = range(0, len(segments_to_process), batch_size)
                for i in tqdm(batch_iterator, desc="Verarbeite Audio-Batches"):
                    batch_segments = segments_to_process[i:i + batch_size]
                    
                    # Innere Schleife für die Inferenz pro Segment innerhalb des Batches
                    for j, segment_info in enumerate(batch_segments):
                        text = segment_info['text']
                        
                        tqdm.write(f"Synthetisiere: \n{segment_info['startzeit']} -> {segment_info['endzeit']}:\n{text}\"")
                        
                        with torch.inference_mode():
                            # Aufruf der optimierten .inference()-Methode
                        ...
                        chunk_index = processed_count + i + j
                        ...
                        # Fortschritt im Manifest speichern (nach jedem Segment)
                        with open(TTS_PROGRESS_MANIFEST, mode='a', encoding='utf-8', newline='') as f:
                            writer = csv.writer(f, delimiter='|')
                            if os.path.getsize(TTS_PROGRESS_MANIFEST) == 0:
                                writer.writerow(['startzeit', 'endzeit', 'text', 'chunk_path'])
                            writer.writerow([segment_info['startzeit'], segment_info['endzeit'], segment_info['text'], chunk_path])

        # Stufe 2: FFmpeg-Assemblierung basierend auf Zeitstempeln
        print("\n-------------------------------------------")
        print("|<< Starte finale Audio-Assemblierung    >>|")
        print("-------------------------------------------")
        ...
        if not final_manifest:
            logger.error("Keine gültigen Audio-Chunks zum Zusammenfügen im Manifest gefunden. Breche ab.")
            return
...
#    detect_speech(PROCESSED_AUDIO_PATH_SPEED, ONLY_SPEECH)
    ...
    segments = transcribe_audio_with_timestamps_safe(ORIGINAL_AUDIO_PATH, TRANSCRIPTION_FILE)
    if not segments:
