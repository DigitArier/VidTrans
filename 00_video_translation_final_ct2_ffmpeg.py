...
def setup_logging():
    """
    NEU: Kapselt die gesamte Logging-Konfiguration.
...
def load_madlad400_translator_ct2(model_path: str, device: str):
    """
...
def translate_batch_ct2(
    ...
    # Schritt 1: Rohtexte vorbereiten (Präfix + Tokenisierung)
    source_texts_with_prefix = [f"<2{target_lang}> {text.strip()}" for text in texts]
    source_tokenized = tokenizer(source_texts_with_prefix, padding=True, truncation=True, max_length=512).input_ids

    # Konvertiere die Token-IDs zurück in Strings für CTranslate2
    source_tokens_as_strings = [tokenizer.convert_ids_to_tokens(ids) for ids in source_tokenized]
    
    # Schritt 2: Inferenz mit der .translate_batch()-Methode der CT2-Engine
    # WICHTIG: `target_prefix` ist für mehrsprachige Modelle wie MADLAD notwendig.
    # Das <2de>-Token wird als Startpunkt für den Decoder verwendet.
    # Für T5-Modelle muss das `target_prefix` mit einem Padding-Token beginnen.
    target_prefix = [[tokenizer.pad_token, f"<2{target_lang}>"]] * len(source_tokens_as_strings)
    ...
    translations = tokenizer.batch_decode(translated_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)
...
def translate_segments_optimized_safe(
    ...
    segments_to_process = []
    # Verwende das bereits vorhandene Entity-Mapping aus der CSV
    for i, row in df_source.iterrows():
        if row['startzeit'] not in processed_keys:
            ...
            # Validierung des Mappings (leeres Dict erlauben, aber loggen)
            ...
            # Schutz direkt mit vorhandenem Mapping anwenden
    ...
    with gpu_context():
        ...
        ct2_model_path = "madlad400-3b-mt-int8_bfloat16" # Pfad zum konvertierten Modell
        translator, tokenizer = load_madlad400_translator_ct2(ct2_model_path, "cuda")
        ...
        for i in tqdm(range(0, len(segments_to_process), batch_size), desc="Übersetze geschützte Batches"):
            ...
            translated_protected_texts = translate_batch_ct2(
...
def entity_protection_final(text: str, nlp_model, tokenizer: T5TokenizerFast) -> Tuple[str, Dict[str, EntityInfo]]:
    """
...
def refine_text_pipeline(
    ...
    with gpu_context():
        ...
        #special_tokens_to_add += [f"<2{lang}>" for lang in ["de", "en", "es", "fr", "ru"]]
    ...
    del nlp_model, punctuation_model, tokenizer
...
def main():
    ...
    """
    try:
        madlad_model_path = "madlad400-3b-mt" # ANPASSEN!
        
        # Die 100 Sentinel-Tokens und die Sprach-Präfixe generieren
        tokens_to_inject = [f"<extra_id_{i}>" for i in range(100)]
        tokens_to_inject += [f"<2{lang}>" for lang in ["de", "en", "es", "fr", "ru"]]
        
        create_and_inject_added_tokens(madlad_model_path, tokens_to_inject)
        
    except Exception as e:
        logger.error(f"Fehler bei der einmaligen Token-Injektion: {e}")
        return # Im Fehlerfall abbrechen
    """
