...
def setup_global_torch_optimizations():
    ...
    try:
        ...
        if sys.platform == "win32" and ctypes.windll.shell32.IsUserAnAdmin():
            ...
            thread_count = min(len(p_cores), 8) # Setzt die Thread-Anzahl auf die Anzahl der P-Cores.
...
def clear_spacy_cache_and_free_vram() -> None:
    """Entfernt explizit Modelle aus dem globalen spaCy-Cache und ruft
    anschließend comprehensive_gpu_cleanup() auf (nicht nur empty_cache).

    Neu: Leert zusätzlich thinc-interne Referenzen via cupy-Pool (falls
    verfügbar) und erzwingt 4 GC-Runden für zirkuläre Referenzen.
    """
    global _SPACY_MODELS_CACHE

    logger.info(
        "Starte explizite Entleerung des spaCy-Modell-Caches und VRAM-Freigabe."
    )

    # --- 1. Alle spaCy-Modelle aus Cache + Speicher entfernen ---
    if _SPACY_MODELS_CACHE and isinstance(_SPACY_MODELS_CACHE, dict):
        for model_name, model_instance in list(_SPACY_MODELS_CACHE.items()):
            logger.info(f"Entferne spaCy-Modell '{model_name}' aus dem Cache.")

            # thinc-interne Gewichte explizit löschen
            try:
                if hasattr(model_instance, "vocab") and hasattr(
                    model_instance.vocab, "lookups"
                ):
                    model_instance.vocab.lookups._tables.clear()
            except Exception:
                pass

            # Alle Pipeline-Komponenten (tok2vec, ner, …) entladen
            try:
                for pipe_name in list(model_instance.pipe_names):
                    pipe = model_instance.get_pipe(pipe_name)
                    if hasattr(pipe, "model"):
                        pipe.model.finish_update(
                            None  # type: ignore[arg-type]
                        ) if hasattr(pipe.model, "finish_update") else None
                        del pipe.model
            except Exception:
                pass
    ...
    # --- 2. thinc cupy Memory-Pool leeren (falls CUDA via cupy) ---
    try:
        from thinc.api import get_current_ops

        ops = get_current_ops()
        if hasattr(ops, "xp") and hasattr(ops.xp, "get_default_memory_pool"):
            ops.xp.get_default_memory_pool().free_all_blocks()
            logger.info("thinc cupy Memory-Pool geleert.")
    except Exception:
        pass  # cupy nicht installiert → ignorieren

    # --- 3. Mehrfache GC-Runden ---
    for _round in range(4):
        gc.collect()

    # --- 4. Umfassende GPU-Bereinigung (bereits im Skript definiert) ---
    comprehensive_gpu_cleanup()  # macht sync + mehrfach empty_cache + ipc_collect

def purge_all_vram_before_translation() -> None:
    """Letzter VRAM-Flush unmittelbar vor EXECUTE_TRANSLATION.

    Sicherheitsnetz: Entfernt alle noch verbleibenden Tensor-Fragmente,
    die weder durch refine_text_pipeline noch clearspacycacheandfreevram
    erfasst wurden (z. B. durch Ollama-Puffer, sentence-transformers
    oder torch-interne Pinned-Memory-Blöcke).

    VRAM ~0 GB nach Aufruf (Allocated); Reserved ≤ 0.1 GB akzeptabel.
    """
    logger.info("=" * 60)
    logger.info("PURGE: Starte vollständige VRAM-Bereinigung vor Translation")
    logger.info("=" * 60)

    if not torch.cuda.is_available():
        logger.warning("CUDA nicht verfügbar – Bereinigung übersprungen.")
        return

    # Schritt 1: Alle ausstehenden CUDA-Ops abwarten
    torch.cuda.synchronize()

    # Schritt 2: Pinned-Memory-Buffer freigeben
    try:
        torch.cuda.caching_allocator_delete(
            torch.cuda.caching_allocator_alloc(1)
        )
    except Exception:
        pass

    # Schritt 3: 5 Runden GC + empty_cache (zirkuläre Refs auflösen)
    for _i in range(5):
        gc.collect()
        torch.cuda.empty_cache()

    # Schritt 4: IPC-Handles (geteilter Speicher zwischen Prozessen)
    torch.cuda.ipc_collect()

    # Schritt 5: Statistiken zurücksetzen
    torch.cuda.reset_peak_memory_stats()
    torch.cuda.reset_accumulated_memory_stats()

    # Schritt 6: Finaler Status-Log
    allocated_gb = torch.cuda.memory_allocated() / 1024**3
    reserved_gb = torch.cuda.memory_reserved() / 1024**3
    logger.info(
        f"PURGE abgeschlossen → Allocated: {allocated_gb:.3f} GB | "
        f"Reserved: {reserved_gb:.3f} GB"
    )

    # Warnung falls noch zu viel belegt
    if allocated_gb > 0.1:
        logger.warning(
            f"VRAM Allocated > 100 MB ({allocated_gb:.3f} GB) – "
            "prüfe ob ein Modell-Handle nicht freigegeben wurde!"
        )
    else:
        logger.info("✓ VRAM ist bereit für EXECUTE_TRANSLATION.")
...
def load_xtts_v2():
    ...
    config = XttsConfig()
    config.load_json(r"C:\Users\regme\Desktop\Modelle\v203\config.json")
    ...
    xtts_model.load_checkpoint(
        config,
        checkpoint_dir=r"C:\Users\regme\Desktop\Modelle\v203",  # Pfad anpassen
        use_deepspeed=False
    )
...
def load_omnivoice() -> OmniVoice:
    """Lädt das OmniVoice-Modell als Drop-in-Ersatz für load_xtts_v2().

    Unterschiede zu XTTS:
    - Kein zweistufiges Laden (XttsConfig → load_checkpoint)
    - Kein separates Vocoder-Modell nötig
    - BF16 statt FP32 → ~50 % weniger VRAM bei gleicher Qualität
    - device_map="auto" aktiviert CPU-Offloading falls VRAM knapp

    Returns:
        OmniVoice: Vollständig geladenes, eval-bereites Modell.

    Raises:
        RuntimeError: Bei kritischem VRAM-Mangel.
        Exception: Bei Download- oder Initialisierungsfehlern.
    """
    ...
    if torch.cuda.is_available():
        ...
        # Freien VRAM vor dem Laden messen
        free_bytes = (
            torch.cuda.get_device_properties(0).total_memory
            - torch.cuda.memory_reserved(0)
        )
        free_gb = free_bytes / 1024**3
        logger.info(f"Freier VRAM vor OmniVoice-Laden: {free_gb:.2f} GB")

        # Warnschwelle: BF16-Modell + Aktivierungen brauchen ~4 GB
        if free_gb < 4.0:
            logger.warning(
                f"Freier VRAM ({free_gb:.2f} GB) < 4 GB – CPU-Offloading "
                "über device_map='auto' aktiv. Synthese wird langsamer."
            )
    ...
    try:
        ...
        if torch.cuda.is_available():
            allocated_gb = torch.cuda.memory_allocated() / 1024**3
            logger.info(
                f"✅ OmniVoice geladen. VRAM belegt: {allocated_gb:.2f} GB"
            )
        else:
            logger.info("✅ OmniVoice geladen (CPU-Modus).")

        return omnivoice_model

    except Exception as e:
        logger.error(
            f"Kritischer Fehler beim Laden von OmniVoice: {e}", exc_info=True
        )
        raise
...
def translate_batch_madlad(
    ...
    """Batch-Übersetzung mit MADLAD-CT2 — VRAM-optimiert ohne STS-Scoring.

    Generiert ``num_hypotheses`` Übersetzungskandidaten pro Segment und wählt
    provisorisch den besten mittels normiertem Beam-Score aus.
    Die finale STS-basierte Hypothesenauswahl erfolgt **entkoppelt** in
    ``select_best_hypotheses_post_translation()``, nachdem der Translator
    entladen wurde und der VRAM frei ist.

    Vorteile gegenüber der gekoppelten Version:
    - Kein STS-Modell (~0.4 GB) parallel zum Translator im VRAM
    - Kürzere GPU-Belegungszeit pro Batch
    - Saubere Trennung: Übersetzung → Entladen → STS-Auswahl
    ...
    Returns:
        Tuple aus:
        - Liste der provisorischen Übersetzungen (beste per Beam-Score).
    ...
    """
    logger.info(
        "Starte Multi-Hypothesis MADLAD-Übersetzung (ohne STS): %d Segmente, "
        "%d Hypothesen",
        len(texts),
        num_hypotheses,
    )
    ...
    # --- Schritt 4: Provisorische Auswahl nur per Beam-Score ---
    # STS-Scoring erfolgt entkoppelt in select_best_hypotheses_post_translation()
    ...
    for seg_idx, seg_hyps in enumerate(decoded_hypotheses):
        ...
        # Provisorisch: Hypothese mit höchstem normiertem Beam-Score wählen
        best_idx = int(np.argmax(scaled_beam))
        best_text, best_raw_score, best_n_tokens = seg_hyps[best_idx]
        provisional_translations.append(best_text)

        logger.debug(
            "Segment %d: provisorisch beste Hypothese idx=%d, "
            "beam_norm=%.4f, text='%s...'",
            segment_id,
            best_idx,
        ...
        # CSV-Datenstruktur: score = normierter Beam-Score, sts_score = 0.0
        # (wird von select_best_hypotheses_post_translation() nachträglich befüllt)
        segment_data = {
            "segment_id": segment_id,
            "source_text": source_text,
            "hypotheses": [
    ...
    # --- Schritt 5: Hypothesen-CSV speichern ---
    ...
    logger.info(
        "Multi-Hypothesis-Übersetzung abgeschlossen (provisorisch per Beam-Score): "
        "%d Segmente, Hypothesen gespeichert in '%s'.",
...
def select_best_hypotheses_post_translation(
    ...
    Liest die flache Hypothesen-CSV (eine Zeile pro Hypothese) und berechnet
    einen kombinierten Score aus:
    - Normiertem Beam-Score (``beam_score_norm`` aus CSV, Modell-Konfidenz)
    - STS-Cosine-Similarity Quelltext → Hypothese (semantische Treue)

    Das STS-Modell wird erst hier geladen — nach dem Entladen des
    MADLAD-Translators —, sodass kein VRAM-Konflikt entsteht.

    Performance-Optimierungen:
    - Batch-STS-Encoding über alle Segmente (3–8× schneller als Einzel-Encoding)
    - Diagonale der Cosine-Similarity-Matrix statt voller Matrixberechnung
    - VRAM-Cleanup nach Abschluss

    Args:
        hypotheses_csv_path: Pfad zur Hypothesen-CSV aus translate_batch_madlad().
        translation_output_file: Pfad zur provisorischen Übersetzungs-CSV.
        report_output_path: Pfad für den Auswahl-Bericht.
    ...
    Returns:
        Pfad zur semantisch optimierten Übersetzungs-CSV.
    """
    logger.info(
        "Starte entkoppelte Hypothesen-Auswahl (Beam+STS, alpha=%.2f)...",
    ...
    # Prüfen, ob beam_score_norm-Spalte vorhanden ist (für kombinierten Score)
    has_beam_norm = 'beam_score_norm' in df_hyp.columns
    if not has_beam_norm:
        logger.warning(
            "Spalte 'beam_score_norm' fehlt in Hypothesen-CSV. "
            "Verwende nur STS-Score für Auswahl (alpha wird ignoriert)."
        )
    ...
    for _, row in df_hyp.iterrows():
        ...
        if has_beam_norm:
            hyp_entry['beam_score_norm'] = (
                float(row['beam_score_norm']) if row['beam_score_norm'] else 0.0
            )
        segments_with_hypotheses[seg_id]['hypotheses'].append(hyp_entry)
    ...
    # --- Batch-STS-Encoding vorbereiten ---
    # Alle Hypothesentexte und zugehörige Quelltexte in flache Listen sammeln
    seg_id_order = sorted(segments_with_hypotheses.keys())
    all_hyp_texts: List[str] = []
    all_src_texts_repeated: List[str] = []
    seg_hyp_counts: List[Tuple[int, int]] = []  # (seg_id, n_hypotheses)

    for seg_id in seg_id_order:
        seg_data = segments_with_hypotheses[seg_id]
        source_text = seg_data['source_text']
        hypotheses = seg_data['hypotheses']
        seg_hyp_counts.append((seg_id, len(hypotheses)))
        for h in hypotheses:
            all_hyp_texts.append(h['text'])
            all_src_texts_repeated.append(source_text)
    ...
    with gpu_context():
        ...
        try:
            ...
            # Alle Texte in einem einzigen Batch encoden (3–8× schneller)
            ...
            # Paarweise Cosine-Similarity (Diagonale der Matrix)
            pairwise_sim = cos_sim(hyp_embeddings, src_embeddings)
            pairwise_scores = pairwise_sim.diagonal().cpu().numpy()

        except Exception as e:
            logger.error(
                "STS-Scoring fehlgeschlagen: %s. Fallback auf Beam-Score.",
                e,
                exc_info=True,
            )
            pairwise_scores = None
        finally:
            if 'similarity_model' in locals() and similarity_model is not None:
                del similarity_model
                torch.cuda.empty_cache()
                gc.collect()

    # --- Kombinierten Score berechnen und beste Hypothese pro Segment wählen ---
    offset = 0
    for seg_id, n_hyps in seg_hyp_counts:
        seg_data = segments_with_hypotheses[seg_id]
        source_text = seg_data['source_text']
        hypotheses = seg_data['hypotheses']
        ...
        if not source_text or not hypotheses:
            ...
            best_translations_details[seg_id] = {
                ...
                'similarity': 0.0,
                'combined_score': best_hyp.get('score', 0.0),
                'source_text': source_text,
            }
            offset += n_hyps
            continue

        # STS-Scores für dieses Segment extrahieren
        seg_sts_scores = (
            pairwise_scores[offset : offset + n_hyps].tolist()
            if pairwise_scores is not None
            else None
        )

        # Beam-Scores normieren (min-max innerhalb Segment auf [0, 1])
        if has_beam_norm:
            raw_beam = [h['beam_score_norm'] for h in hypotheses]
            beam_min = min(raw_beam)
            beam_max = max(raw_beam)
            beam_range = beam_max - beam_min if beam_max != beam_min else 1.0
            scaled_beam = [(s - beam_min) / beam_range for s in raw_beam]
        else:
            scaled_beam = None

        # Kombinierten Score berechnen
        combined_scores: List[float] = []
        for hyp_idx, h in enumerate(hypotheses):
            if scaled_beam is not None and seg_sts_scores is not None:
                # Voller kombinierter Score: alpha * Beam + (1-alpha) * STS
                beam_component = scaled_beam[hyp_idx] * hypothesis_score_alpha
                sts_component = float(seg_sts_scores[hyp_idx]) * (1.0 - hypothesis_score_alpha)
                combined_scores.append(beam_component + sts_component)
            elif seg_sts_scores is not None:
                # Kein Beam-Score → nur STS
                combined_scores.append(float(seg_sts_scores[hyp_idx]))
            elif scaled_beam is not None:
                # Kein STS → nur Beam-Score
                combined_scores.append(scaled_beam[hyp_idx])
            else:
                # Weder STS noch Beam → Fallback auf CSV-Score
                combined_scores.append(h['score'])
        ...
        best_translations_details[seg_id] = {
            ...
            'similarity': seg_sts_scores[best_idx] if seg_sts_scores else 0.0,
            'combined_score': combined_scores[best_idx],
            'source_text': source_text,
            'all_hypotheses_with_scores': sorted(
                zip(hypothesis_texts, combined_scores),
                key=lambda x: x[1],
                reverse=True,
            ),
        }
        ...
        logger.debug(
            ...
            "beam=%.4f, sts=%.4f, text='%s...'",
            seg_id,
            best_idx,
            combined_scores[best_idx],
            scaled_beam[best_idx] if scaled_beam else -1.0,
            seg_sts_scores[best_idx] if seg_sts_scores else -1.0,
            hypotheses[best_idx]['text'][:60],
        )

        offset += n_hyps

    # --- Ergebnis in Übersetzungs-CSV schreiben ---
...
VOCABULARY_HINTS: dict[str, str] = {
    # ── RELIGIÖS / HISTORISCH ───────────────────────────────
    "abomination": "Abscheulichkeit, Gräuel, Scheußlichkeit",
    ...
    "the Fall": "der Sündenfall",
    "scripture": "die Heilige Schrift, Bibelvers",
    "covenant": "Bund, Bündnis (religiös: Bundesvertrag)",
    "prophecy": "Prophezeiung, Weissagung",
    "prophet": "Prophet",
    "disciple": "Jünger, Schüler",
    "apostle": "Apostel",
    "gospel": "Evangelium",
    "salvation": "Erlösung, Heil",
    "repentance": "Buße, Reue, Umkehr",
    "sin": "Sünde",
    "sinner": "Sünder",
    "grace": "Gnade",
    "wrath": "Zorn, Wrath (nie: Ärger)",
    "judgment": "Gericht, Urteil (religiös: Jüngstes Gericht)",
    "tribulation": "Drangsal, Trübsal, Bedrängnis",
    "rapture": "Entrückung (theol.), Verzückung",
    "Antichrist": "Antichrist",
    "End Times": "Endzeit, die letzten Tage",
    ...
    "globalist": "Globalist",
    "establishment": "Establishment, herrschende Klasse",
    "propaganda": "Propaganda",
    "narrative": "Narrativ, Erzählung (nie: Geschichte)",
    "mainstream media": "Mainstream-Medien, Leitmedien",
    "whistleblower": "Enthüller, Hinweisgeber, Whistleblower",
    "cover-up": "Vertuschung",
    "false flag": "False-Flag-Operation, Täuschungsanschlag",

    # ── FALSE FRIENDS (häufigste Fehler) ────────────────────
    "eventually": "schließlich, letztendlich  ← NICHT: eventuell",
    "actual": "tatsächlich, wirklich  ← NICHT: aktuell",
    "actually": "tatsächlich, eigentlich  ← NICHT: aktuell",
    "currently": "derzeit, momentan  ← NICHT: aktuell (das ist auch möglich aber kontextabhängig)",
    "sensible": "vernünftig, sinnvoll  ← NICHT: sensibel",
    "sensitive": "empfindlich, sensibel  ← NICHT: vernünftig",
    "familiar": "vertraut, bekannt  ← NICHT: familiär",
    "sympathetic": "mitfühlend, verständnisvoll  ← NICHT: sympathisch",
    "pathetic": "erbärmlich, kläglich  ← NICHT: pathetisch",
    "professor": "Professor  ← NICHT: Lehrer",
    "gymnasium": "Turnhalle  ← NICHT: Gymnasium (das ist eine Schule)",
    "receipt": "Quittung, Kassenbon  ← NICHT: Rezept",
    "recipe": "Rezept  ← NICHT: Rezept für Arzt",
    "chef": "Küchenchef, Koch  ← NICHT: Chef (Vorgesetzter)",
    "boss": "Chef, Vorgesetzter  ← NICHT: Chef (Küche)",
    "gift": "Geschenk  ← NICHT: Gift (das ist Poison)",
    "poison": "Gift  ← NICHT: Geschenk",
    "blank": "leer, unbeschrieben  ← NICHT: blank (nackt)",
    "fabric": "Stoff, Gewebe  ← NICHT: Fabrik",
    "factory": "Fabrik  ← NICHT: Stoff",
    "stiff": "steif, starr  ← NICHT: stiff (kein deutsches Wort)",
    "brave": "mutig, tapfer  ← NICHT: brav (gehorsam)",
    "brav": "DEUTSCH: gehorsam, artig  (englisch: well-behaved)",
    "become": "werden  ← NICHT: bekommen (das ist: receive/get)",
    "bekommen": "DEUTSCH: erhalten, kriegen  (englisch: receive/get)",
    "also": "außerdem, ebenfalls  ← NICHT: also (das ist: so/therefore)",
    "thus": "daher, deshalb, somit",
    "therefore": "daher, deshalb, folglich",
    "hence": "daher, deshalb, von hier aus",
    "though": "obwohl, jedoch, allerdings",
    "although": "obwohl, obgleich",
    "while": "während (zeitlich), wohingegen (konzessiv)",
    "since": "seit (zeitlich), da/weil (kausal – Kontext beachten!)",
    "as": "als, wie, da, weil – Kontext entscheidend",
    "once": "einmal (Häufigkeit), sobald (zeitlich: once he arrived → sobald er ankam)",

    # ── MODALVERBEN / GRAMMATIK-FALLEN ──────────────────────
    "must": "muss (Pflicht)  ← NICHT: musste (Vergangenheit)",
    "had to": "musste, war gezwungen zu",
    "should": "sollte, müsste (Ratschlag)",
    "ought to": "sollte, müsste (moralische Pflicht)",
    "would": "würde (Konditional), wollte (Vergangenheit von will)",
    "could": "konnte, könnte",
    "might": "könnte, dürfte (Möglichkeit)",
    "may": "darf, dürfte, kann (Erlaubnis/Möglichkeit)",
    "dare": "wagen, sich trauen",
    "need not": "muss nicht, braucht nicht zu",
    "used to": "früher (+ Verb), pflegte zu  ← NICHT: gewöhnt sein",
    "be used to": "gewohnt sein, gewöhnt sein an",
    "get used to": "sich gewöhnen an",

    # ── ZAHLENWÖRTER / MENGEN ────────────────────────────────
    "a couple of": "ein paar, einige, zwei  (kontextabhängig)",
    "a few": "einige, wenige",
    "several": "mehrere, einige",
    "numerous": "zahlreich, viele",
    "countless": "unzählige, zahllose",
    "dozens of": "Dutzende von",
    "hundreds of": "Hunderte von",
    "thousands of": "Tausende von",
    "millions of": "Millionen von",
    "billion": "Milliarde  ← NICHT: Billion (das sind 10^12)",
    "trillion": "Billion (10^12)  ← NICHT: Trillion",
    "half": "halb, die Hälfte",
    "double": "doppelt, verdoppeln",
    ...
    "study": "Studie, Untersuchung (nie: studieren als Verb in Substantiv-Form)",
    "research": "Forschung, Untersuchung",
    "evidence": "Beweis, Belege, Hinweise  ← NICHT: Evidenz (veraltet)",
    "proof": "Beweis",
    "theory": "Theorie (wissenschaftlich), Vermutung (umgangssprachlich)",
    "hypothesis": "Hypothese, Annahme",
    "data": "Daten  ← immer Plural in Deutsch",
    "analysis": "Analyse, Auswertung",
    "result": "Ergebnis, Resultat",
    "conclusion": "Schlussfolgerung, Fazit",
    "significant": "bedeutsam, erheblich (wissenschaftlich: signifikant)",
    "substantial": "erheblich, beträchtlich, substanziell",
    "considerable": "beträchtlich, erheblich",
    "fundamental": "grundlegend, fundamental",
    "essential": "wesentlich, unerlässlich, essenziell",
    "crucial": "entscheidend, ausschlaggebend",
    "critical": "kritisch, entscheidend (Kontext!)",
    "significant impact": "erhebliche Auswirkung, signifikanter Einfluss",

    # ── ZEITLICHE AUSDRÜCKE ──────────────────────────────────
    "recently": "kürzlich, vor Kurzem, in letzter Zeit",
    "lately": "in letzter Zeit, neuerdings",
    "formerly": "früher, ehemals, vormals",
    "previously": "zuvor, vorher, früher",
    "subsequently": "anschließend, danach, in der Folge",
    "simultaneously": "gleichzeitig, zur gleichen Zeit",
    "meanwhile": "inzwischen, unterdessen, währenddessen",
    "eventually": "schließlich, letztendlich, irgendwann",
    "immediately": "sofort, unmittelbar, auf der Stelle",
    "instantly": "sofort, augenblicklich",
    "gradually": "allmählich, schrittweise, nach und nach",
    "constantly": "ständig, fortwährend, unablässig",
    "consistently": "konsequent, durchgehend, beständig",
    "repeatedly": "wiederholt, immer wieder",
    "occasionally": "gelegentlich, hin und wieder",
    "rarely": "selten, kaum",
    "never": "niemals, nie",

    # ── ALLTÄGLICHE BERUFE / BEZEICHNUNGEN ──────────────────
    "janitor": "Hausmeister",
    "janitors": "Hausmeister",
    "attorney": "Anwalt, Rechtsanwalt",
    "lawyer": "Anwalt, Jurist",
    "physician": "Arzt, Mediziner  ← NICHT: Physiker",
    "physicist": "Physiker  ← NICHT: Arzt",
    "surgeon": "Chirurg",
    "nurse": "Krankenschwester, Krankenpfleger, Pflegefachkraft",
    "principal": "Schulleiter, Direktor (Schule)  ← NICHT: Prinzipal",
    "dean": "Dekan, Studiendirektor",
    "minister": "Minister (Politik), Pfarrer/Pastor (Kirche) – Kontext!",
    "chancellor": "Bundeskanzler (Deutschland), Kanzler",
    "senator": "Senator",
    "representative": "Abgeordneter, Vertreter",
    "officer": "Offizier (Militär), Beamter (Polizei), Beauftragter",
    "agent": "Agent, Beauftragter, Vertreter",

    # ── EMOTIONEN / PSYCHOLOGIE ──────────────────────────────
    "atrocity": "Gräueltat, Grausamkeit, Scheußlichkeit",
    "atrocious": "grausam, entsetzlich, abscheulich",
    "heinous": "abscheulich, niederträchtig, verabscheuungswürdig",
    "vile": "widerlich, abscheulich, gemein",
    "despicable": "verachtenswert, niederträchtig",
    "grotesque": "grotesk, abstoßend, bizarr",
    "macabre": "makaber, grausig, düster",
    "sinister": "finster, unheilverkündend, bedrohlich",
    "menacing": "bedrohlich, drohend",
    "foreboding": "unheilvolle Ahnung, düstere Vorahnung",
    "ominous": "unheilverkündend, bedrohlich",
    "eerie": "unheimlich, gespenstisch",
    "uncanny": "unheimlich, seltsam merkwürdig",
    "harrowing": "erschütternd, qualvoll, herzzerreißend",
    "excruciating": "qualvoll, unerträglich",
    "agonizing": "qualvoll, schmerzhaft",
    ...
    "merciless": "gnadenlos, erbarmungslos",
    ...
    "audacity": "Kühnheit, Wagemut, Dreistigkeit",
    "resilience": "Widerstandsfähigkeit, Belastbarkeit",
    "fortitude": "Standhaftigkeit, innere Stärke",
    "valor": "Tapferkeit, Heldenmut",
    ...
    "candor": "Offenheit, Aufrichtigkeit",

    # ── NEGATION / INTENSIVIERUNG ────────────────────────────
    "utterly": "vollkommen, völlig, schlechterdings",
    "entirely": "vollständig, gänzlich, völlig",
    "completely": "vollständig, komplett, ganz und gar",
    "absolutely": "absolut, unbedingt, in jeder Hinsicht",
    "undeniably": "unbestreitbar, zweifellos",
    "unquestionably": "zweifelsohne, fraglos",
    "arguably": "wohl, man könnte sagen, möglicherweise",
    "supposedly": "angeblich, vermeintlich",
    "allegedly": "angeblich, dem Vernehmen nach",
    "apparently": "anscheinend, offenbar",
    "seemingly": "scheinbar, dem Anschein nach",
    "ostensibly": "angeblich, vordergründig, dem Anschein nach",

    # ── HÄUFIGE SYNTAX-FALLEN ────────────────────────────────
    "St": "Sankt, Heiliger",
    "saw": "sahen, gesehen",
    "saw from": "sah von, gesehen von",
    "saw that": "sah, dass",
    "there is": "es gibt (kein: es ist ein/eine)",
    "there are": "es gibt (Plural)",
    "it is said": "es heißt, es wird gesagt",
    "it is believed": "es wird angenommen, man glaubt",
    "it is known": "es ist bekannt",
    "as well as": "sowie, als auch, ebenso wie",
    "not only … but also": "nicht nur … sondern auch",
    "either … or": "entweder … oder",
    "neither … nor": "weder … noch",
    "both … and": "sowohl … als auch",
    "in order to": "um … zu (Infinitiv), damit (Nebensatz)",
    "so that": "damit, sodass",
    "due to": "aufgrund (von), wegen",
    "because of": "wegen, aufgrund von",
    "despite": "trotz (Genitiv!)",
    "in spite of": "trotz, ungeachtet",
    "regardless of": "ungeachtet, unabhängig von",
    "according to": "laut, gemäß, nach Angaben von",
    "based on": "basierend auf, auf der Grundlage von",

    # ── VORHER VORHANDENE EINTRÄGE (beibehalten) ────────────
    "bewildering": "verwirrend, rätselhaft, bestürzend",
    "perplexing": "verwirrend, rätselhaft",
    ...
    "obliteration": "Auslöschung, Vernichtung",
    "extermination": "Ausrottung, Vernichtung",
    "eradication": "Ausrottung, Beseitigung",
    "elimination": "Beseitigung, Ausschaltung",
    "culmination": "Höhepunkt, Kulmination",
    "pinnacle": "Gipfel, Höhepunkt",
    "zenith": "Zenit, Höhepunkt",
    "apex": "Spitze, Gipfel",
    ...
    "revelation": "Offenbarung, Enthüllung",
    "epiphany": "Erleuchtung, plötzliche Erkenntnis",
    "enlightenment": "Erleuchtung, Aufklärung",
    "sophistication": "Raffinesse, Kultiviertheit",
    "refinement": "Verfeinerung, Kultiviertheit",
    "elegance": "Eleganz, Anmut",
    "magnificence": "Pracht, Herrlichkeit",
    "grandeur": "Pracht, Großartigkeit, Erhabenheit",
    "majesty": "Majestät, Erhabenheit, Würde",
    "nobility": "Adel, Vornehmheit, Edelmut",
    "transparency": "Transparenz, Offenheit",
    "lucidity": "Klarheit, Verständlichkeit",
    "clarity": "Klarheit, Deutlichkeit",
    "coherence": "Kohärenz, Zusammenhang, Stimmigkeit",
    "consistency": "Konsistenz, Beständigkeit, Folgerichtigkeit",
    "persistence": "Ausdauer, Beharrlichkeit",
    "indomitable": "unbezwingbar, unbesiegbar, unbeugsam",
    "invincible": "unbesiegbar, unverwundbar",
    ...
    "transient": "vorübergehend, vergänglich",
    "fleeting": "flüchtig, vergänglich, kurz",
    "temporary": "vorübergehend, zeitweilig, temporär",
    "provisional": "vorläufig, provisorisch",
    "preliminary": "vorläufig, einleitend",
}

def create_vocabulary_support() -> str:
    """Erstellt einen strukturierten, kategorisierten Vokabel-Support-Text.

    Returns:
        Formatierter String für LLM-System-Prompts mit Kategorien,
        Kontexthinweisen und False-Friend-Warnungen.
    """
    if not VOCABULARY_HINTS:
        return ""

    # Kategorien-Präfixe für strukturierte Ausgabe
    _FALSE_FRIEND_MARKER = "← NICHT:"
    _GERMAN_MARKER = "DEUTSCH:"

    false_friends: list[str] = []
    regular_hints: list[str] = []

    for english_word, german_options in sorted(VOCABULARY_HINTS.items()):
        if _FALSE_FRIEND_MARKER in german_options or _GERMAN_MARKER in german_options:
            false_friends.append(f"  {english_word!r:30s} → {german_options}")
        else:
            regular_hints.append(f"  {english_word!r:30s} → {german_options}")

    lines: list[str] = [
        "══════════════════════════════════════════════════════",
        "  TRANSLATION VOCABULARY REFERENCE  (authoritative)",
        "══════════════════════════════════════════════════════",
        "",
        "⚠️  FALSE FRIENDS  – These English words are OFTEN mistranslated:",
        "   (The correct German equivalent follows the arrow)",
        "─────────────────────────────────────────────────────",
    ]
    lines.extend(false_friends)

    lines += [
        "",
        "📖  VOCABULARY HINTS  – Preferred German translations by context:",
        "─────────────────────────────────────────────────────",
    ]
    lines.extend(regular_hints)

    lines += [
        "",
        "══════════════════════════════════════════════════════",
        "USAGE RULES:",
        "  1. False Friends → ALWAYS use the warned translation.",
        "  2. Vocabulary Hints → choose the option that best fits",
        "     the REGISTER and CONTEXT of the segment.",
        "  3. When in doubt: prefer the SHORTER, MORE COMMON option.",
        "  4. NEVER translate proper nouns, acronyms, or units",
        "     unless they appear in this list.",
        "══════════════════════════════════════════════════════",
    ]
...
def evaluate_translation_quality(
    ...
    model_name: str,
    threshold: float,
    correction_model: str = "qwen3:8b",
    max_retries: int = 3,
) -> tuple[str, str]:
    """Prüft semantische Ähnlichkeit, korrigiert niedrige Segmente automatisch
    via Ollama und erzeugt detaillierten Bericht + Zusammenfassung.

    Args:
        source_csv_path: Pfad zur Quell-CSV (EN).
        translated_csv_path: Pfad zur übersetzten CSV (DE).
        report_path: Ausgabepfad für den Qualitätsbericht.
        summary_path: Ausgabepfad für die Zusammenfassung.
        model_name: Sentence-Transformer-Modell für Similarity.
            Empfohlen: 'sentence-transformers/LaBSE' (cross-lingual).
        threshold: Similarity-Schwelle; Segmente darunter → Korrektur.
        correction_model: Ollama-Modell für automatische Korrekturen.
        max_retries: Maximale Korrekturversuche pro Segment.

    Returns:
        Tuple[str, str]: (report_path, corrected_csv_path)

    Raises:
        ValueError: Wenn Pflicht-Spalten in den CSVs fehlen.
    """
    # VRAM ~1.2 GB für LaBSE – sicher innerhalb des 6 GB Limits
    ...
    try:
        # ── Daten laden ────────────────────────────────────────
        with tqdm(total=1, desc="CSV-Dateien laden",
                  bar_format="{l_bar}{bar}| {elapsed}") as pbar:
            df_source = pd.read_csv(
                source_csv_path, sep="|", dtype=str
            ).fillna("")
            df_translated = pd.read_csv(
                translated_csv_path, sep="|", dtype=str
            ).fillna("")
            pbar.update(1)
        ...
        if len(df_source) != len(df_translated):
            logger.warning(
                ...
                f"{len(df_source)} vs {len(df_translated)}. "
        ...
        df_translated.columns = [
            col.strip().lower() for col in df_translated.columns
        ]

        if "text" not in df_source.columns or "text" not in df_translated.columns:
            logger.error(
                "Benötigte Spalte 'text' nicht in den CSVs gefunden."
            )
            return None, None  # type: ignore[return-value]

        source_texts: list[str] = df_source["text"].tolist()
        translated_texts: list[str] = df_translated["text"].tolist()

        # ── Modell laden & Embeddings berechnen ───────────────
        # VRAM ~1.2 GB (LaBSE 471 M Parameter, float32 auf GPU)
        with gpu_context():
            print(f"Lade Sentence Transformer: {model_name}")
            print("Dies kann 5–30 Sekunden dauern ...")
            with tqdm(
                total=1,
                desc="Modell initialisieren",
            ...
            print(f"Berechne Embeddings für {len(source_texts)} Segmente ...")
            embeddings_source = model.encode(
                source_texts, convert_to_tensor=True, show_progress_bar=True
            )
            embeddings_translated = model.encode(
                translated_texts, convert_to_tensor=True, show_progress_bar=True
            )

            print("Berechne semantische Ähnlichkeiten ...")
            with tqdm(
                total=1,
                desc="Similarity-Matrix",
                bar_format="{l_bar}{bar}| {elapsed}",
            ) as pbar:
                similarities: list[float] = torch.diag(
                    cos_sim(embeddings_source, embeddings_translated)
                ).tolist()
                pbar.update(1)

            # VRAM freigeben – Modell auf CPU verschieben
            model = model.to("cpu")
            embeddings_source = embeddings_source.to("cpu")
            torch.cuda.empty_cache()

        # ── Vokabular-Support erstellen ────────────────────────
        print("Erstelle Vocabulary-Support ...")
        with tqdm(
            total=1,
            desc="VOCABULARY_HINTS formatieren",
            bar_format="{l_bar}{bar}| {elapsed}",
        ) as pbar:
        ...
        auto_corrected_segments: list[dict] = []

        segments_to_correct_count = sum(
            1 for sim in similarities if sim < threshold
        )
        correction_pbar = tqdm(
            total=segments_to_correct_count,
        ...
        CLIFFHANGER_ENDINGS: tuple[str, ...] = (
            ...
            "der.", "die.", "das.", "den.", "dem.",
            "that.", "which.", "who.", "but.", "and.", "because.",
            "wenn.", "als.", "obwohl.", "damit.", "sodass.",
        )
        ...
        for i in range(len(df_source)):
            ...
            if similarity < threshold:
                ...
                tqdm.write(
                    f"  Segment {i} zur Korrektur markiert | "
                    f"Ähnlichkeit: {similarity:.3f} < {threshold}"
                )
                tqdm.write(f"  Original EN: {source_texts[i]}")
                tqdm.write(f"  Übersetzung DE: {translated_texts[i]}")
                tqdm.write("─" * 80)

                # ── Cliffhanger-Detektion ──────────────────────
                detail = translated_texts[i].strip().lower()
                entail = source_texts[i].strip().lower()
                is_cliffhanger = any(
                    detail.endswith(end) for end in CLIFFHANGER_ENDINGS
                ) or any(
                    entail.endswith(end) for end in CLIFFHANGER_ENDINGS
                )

                if is_cliffhanger:
                    tqdm.write(
                        f"  Segment {i} ist ein Cliffhanger-Satz "
                        f"(endet mit: {translated_texts[i].strip().split()[-1]})\n"
                        "  Korrektur übersprungen, Original beibehalten."
                    )
                    final_corrected_text = translated_texts[i]
                    correction_note = (
                        "Cliffhanger-Segment absichtlich unvollständig, "
                        "keine Korrektur möglich."
                    )
                    flag = "CLIFFHANGER-SKIP"
                    status = f"Cliffhanger ({similarity:.3f})"
                    issues_found += 1
                    results.append({
                        original_source_columns[0]: df_source.iloc[i].get(
                            original_source_columns[0].lower(), "N/A"
                        ),
                        original_source_columns[1]: df_source.iloc[i].get(
                            original_source_columns[1].lower(), "N/A"
                        ),
                        "Quelltext": source_texts[i],
                        "Uebersetzung": final_corrected_text,
                        "Aehnlichkeit": f"{similarity:.4f}",
                        "KorrigierteAehnlichkeit": "",
                        "Status": status,
                ...
                # ── Retry-Mechanismus mit verbessertem Prompt ─
                success = False
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        tqdm.write(
                            ...
                            f"Modell: {correction_model} | "
                            f"Temp: {current_temperature}"
                        )

                        # ──────────────────────────────────────────────────
                        # VERBESSERTER SYSTEM-PROMPT v2.1
                        # Neu: Chain-of-Thought-Struktur + Few-Shot-Beispiele
                        # ──────────────────────────────────────────────────
                        system_prompt = f"""\
You are a senior German localization expert and professional subtitle editor \
specializing in voice-over (single narrator, neutral documentary register).

═══════════════════════════════════════════════════════════
YOUR TASK
═══════════════════════════════════════════════════════════
Receive an English subtitle segment and a flawed German translation.
Produce the single BEST corrected German translation.

Think through these steps SILENTLY (do NOT output them):
  Step 1 – Identify the CORE MEANING of the English source.
  Step 2 – Find where the German translation deviates (False Friends,
           wrong register, unnatural word order, missing/added info).
  Step 3 – Apply the correction rules below.
  Step 4 – Check the character limit ({max_chars} chars).
  Step 5 – Output ONLY the corrected German text.

═══════════════════════════════════════════════════════════
CORRECTION RULES  (priority order)
═══════════════════════════════════════════════════════════
1. SEMANTIC FIDELITY
   - Never add, remove, or alter information.
   - Preserve grammatical mood (indicative stays indicative).
   - Historical present in EN → historical present in DE
     ("Er geht", not "Er ging").

2. FALSE FRIENDS  (most common errors – always fix these)
   - "eventually" → "schließlich"  (NEVER "eventuell")
   - "actual/actually" → "tatsächlich"  (NEVER "aktuell")
   - "become" → "werden"  (NEVER "bekommen")
   - "billion" → "Milliarde"  (NEVER "Billion")
   - "sympathetic" → "mitfühlend"  (NEVER "sympathisch")
   - "sensible" → "vernünftig"  (NEVER "sensibel")
   - "fabric" → "Stoff/Gewebe"  (NEVER "Fabrik")

3. NATURAL GERMAN GRAMMAR
   - V2 word order in main clauses (verb at position 2).
   - Verb-final in subordinate clauses ("weil er es wusste.").
   - Active voice preferred over passive.
   - "there is/are" → "Es gibt" (never "Es ist ein").
   - English gerund "by doing X" → German "indem sie X tun".
   - Konjunktiv II only when the English uses conditional/subjunctive.

4. CONCISENESS FOR TTS/SUBTITLES
   - Hard limit: {max_chars} characters.
   - Prefer shorter synonyms: nutzen > verwenden, zeigen > demonstrieren.
   - Break compound nouns > 18 chars: "Qualitätsmanagementsystem"
     → "System für Qualitätsmanagement".
   - Split one long sentence into two short ones if clarity improves.

5. SPOKEN RHYTHM
   - Avoid dental clusters and hard consonant chains at phrase ends.
   - Prefer flowing syllable patterns over staccato noun stacks.

═══════════════════════════════════════════════════════════
FEW-SHOT EXAMPLES
═══════════════════════════════════════════════════════════
EN: "The government eventually admitted the cover-up."
BAD DE: "Die Regierung hat eventuell die Vertuschung zugegeben."
GOOD DE: "Die Regierung räumte schließlich die Vertuschung ein."

EN: "There are three main reasons for this phenomenon."
BAD DE: "Es ist drei Hauptgründe für dieses Phänomen."
GOOD DE: "Es gibt drei Hauptgründe für dieses Phänomen."

EN: "Billions of people are affected."
BAD DE: "Billionen von Menschen sind betroffen."
GOOD DE: "Milliarden Menschen sind betroffen."

EN: "She became a leading scientist."
BAD DE: "Sie bekam eine führende Wissenschaftlerin."
GOOD DE: "Sie wurde eine führende Wissenschaftlerin."

═══════════════════════════════════════════════════════════
{vocabulary_support}
═══════════════════════════════════════════════════════════
OUTPUT FORMAT
- Return ONLY the corrected German text.
- No labels, no quotes, no explanations, no pre/post-text.
...
"""
                        ...
                        user_prompt = (
                            ...
                            f"OUTPUT: The corrected German text only. "
                        ...
                        # Markdown-Guard: Entferne alles vor der ersten deutschen Zeile ohne Sonderzeichen
                        def strip_llm_analysis(raw: str, source_de: str) -> str:
                            """Extrahiert die reine Übersetzung aus einem ggf. Markdown/Thinking-angereicherten LLM-Output.

                            Strategie:
                            1. Wenn kein bekannter Artefakt-Marker → direkt zurückgeben (Fast-Exit).
                            2. Sonst: erste Zeile ohne Markdown/Meta, > 20 Zeichen.
                            3. Fallback: source_de zurückgeben.

                            Args:
                                raw: Rohantwort des LLM (bereits .strip()-bereinigt).
                                source_de: Fallback-Text bei leerem/ungültigem Ergebnis.

                            Returns:
                                Bereinigter Text oder source_de.
                            """
                            # Fast-Exit: kein Analyse-Müll erkennbar
                            if not any(
                                marker in raw
                                for marker in ("**", "##", "---", "\n1.", "Error", "<think>", "<analysis>")
                            ):
                                return raw
                            ...
                            candidates = [
                                ...
                                for ln in lines
                                if len(ln.strip()) > 20
                                and not ln.strip().startswith(("#", "*", "-", "|", "<"))  # kein Listenstart, kein XML-Tag
                                and not re.match(r"^\d+\.\s", ln.strip())  # kein "1. Text"-Listenformat
                                and "**" not in ln
                                and "Error" not in ln
                                and "count" not in ln.lower()
                                and "score" not in ln.lower()
                            ...
                            return source_de  # Sicherer Fallback

                        correction_candidate = strip_llm_analysis(
                            response["message"]["content"].strip(),
                            translated_texts[i],  # Fallback
                        )
                        tqdm.write(f"  Korrekturvorschlag: {correction_candidate}")

                        # Similarity des Kandidaten messen
                        # VRAM ~0.1 GB (einzelnes Encoding)
                        new_embedding = model.encode(
                            correction_candidate, convert_to_tensor=True
                        )
                        new_similarity: float = cos_sim(
                        ...
                        if new_similarity > best_similarity + 0.002:
                            ...
                            tqdm.write(
                                f"  Keine signifikante Verbesserung: "
                                f"{similarity:.3f} → {new_similarity:.3f}"
                            )

                    except Exception as e:
                        logger.warning(
                            f"Korrektur-Versuch {attempt + 1} für Segment {i} "
                            f"fehlgeschlagen: {e}",
                            exc_info=True,
                        )
                ...
                if best_similarity > similarity:
                    ...
                    flag = "AUTO-CORRECTED"
                    correction_note = (
                        f"Beste Korrektur nach {max_retries} Versuchen: "
                    ...
                    tqdm.write(
                        f"  ✅ BESTE VERSION ÜBERNOMMEN: "
                        f"{similarity:.3f} → {best_similarity:.3f}"
                    )
                    logger.info(
                        f"Segment {i} erfolgreich korrigiert: "
                        f"{similarity:.3f} → {best_similarity:.3f}"
                    )
                else:
                    final_corrected_text = orig_de
                    correction_note = (
                        f"Keine Verbesserung nach {max_retries} Versuchen."
                    )
                    tqdm.write("  ✗ KEINE VERBESSERUNG, Original beibehalten.")
            ...
            results.append({
                ...
                "Aehnlichkeit": f"{similarity:.4f}",
                "KorrigierteAehnlichkeit": (
                    f"{corrected_similarities[i]:.4f}"
                    if flag == "AUTO-CORRECTED"
                    else ""
                ),
                "Status": status,
                "KorrekturNote": correction_note,
        ...
        avg_similarity_after = (
            sum(corrected_similarities) / len(corrected_similarities)
            if corrected_similarities
            else 0.0
        )

        df_results = pd.DataFrame(results)
        corrected_translation_csv = CORRECTED_TRANSLATION_FILE
        df_corrected = df_translated.copy()
        df_corrected["text"] = [r["Uebersetzung"] for r in results]
        df_corrected.to_csv(
            corrected_translation_csv, sep="|", index=False, encoding="utf-8"
        )

        # Bericht schreiben
        with open(report_path, "w", encoding="utf-8") as f:
            f.write(df_results.to_csv(sep="|", index=False))
        logger.info(f"Qualitätsbericht gespeichert: {report_path}")
        ...
        summary_content = (
            ...
            f"Modell (Similarity):    {model_name}\n"
            f"Modell (Korrektur):     {correction_model}\n"
            f"Schwellenwert:          {threshold}\n"
            f"Quell-Segmente:         {len(source_texts)}\n"
            f"Probleme gefunden:      {issues_found}\n"
            f"Korrekturen erfolgt:    {corrections_made}\n"
            f"Ø Ähnlichkeit vorher:   {avg_similarity_before:.4f}\n"
            f"Ø Ähnlichkeit nachher:  {avg_similarity_after:.4f}\n"
            f"Verbesserung:           "
            f"{(avg_similarity_after - avg_similarity_before) / avg_similarity_before * 100:.1f}%"
            f" (absolut: {avg_similarity_after - avg_similarity_before:+.4f})\n"
            f"\n{'─' * 50}\n"
            f"Auto-korrigierte Segmente:\n"
        )
        for seg in auto_corrected_segments:
            summary_content += (
                f"\n  Segment {seg['index']}: "
                f"{seg['sim_before']:.3f} → {seg['sim_after']:.3f}\n"
                f"  EN: {seg['src_en']}\n"
                f"  DE vorher: {seg['de_before']}\n"
                f"  DE nachher: {seg['de_after']}\n"
            )
        if not auto_corrected_segments:
            summary_content += "  – keine –\n"

        with open(summary_path, "w", encoding="utf-8") as f:
            f.write(summary_content)
        logger.info(f"Qualitäts-Zusammenfassung gespeichert: {summary_path}")
        ...
        logger.error(
            f"Fehler in evaluate_translation_quality: {e}", exc_info=True
        )
        raise
...
def polish_all_translations(
    ...
    Returns:
        Tuple[str, str]: (output_csv_path, summary_path)

    Raises:
        FileNotFoundError: Wenn eine der Eingabe-CSVs nicht existiert.
    """
    # VRAM ~1.2 GB (LaBSE) – sicher innerhalb des 6 GB Limits
    if os.path.exists(summary_path) and not ask_overwrite(summary_path):
        logger.info(f"Verwende vorhandenen Qualitätsbericht: {summary_path}")
        output_csv_path = POLISHED_TRANSLATION_CSV
        return summary_path, output_csv_path
    try:
        # ── Daten laden ────────────────────────────────────────
        df_source = pd.read_csv(
            source_csv_path, sep="|", dtype=str
        ...
        with gpu_context():
            ...
            print(f"Berechne Embeddings für {total_segments} Segmente ...")
            embeddings_source = model.encode(
                source_texts, convert_to_tensor=True, show_progress_bar=True
            )
            embeddings_translated = model.encode(
        ...
        polished_similarities: list[float] = similarities.copy()

        pbarmain = tqdm(total=total_segments, desc="Polishing-Durchlauf")

        # ─────────────────────────────────────────────────────
        # VERBESSERTER POLISHING-SYSTEM-PROMPT v2.1
        # Neu: TTS-Rhythmus, Registerprüfung, Few-Shot
        # ─────────────────────────────────────────────────────
        polishing_system_prompt = f"""\
You are a native German language editor specializing in spoken-word \
localization for documentary and factual content \
(voice-over, single narrator style).

═══════════════════════════════════════════════════════════
YOUR ROLE
═══════════════════════════════════════════════════════════
You receive a semantically correct German subtitle segment and must make \
it MORE NATURAL, MORE CONCISE, and MORE PLEASANT to hear — without \
changing the meaning.

The translation is ALREADY CORRECT. Do NOT fix meaning; fix STYLE only.

═══════════════════════════════════════════════════════════
EDITING PRIORITIES  (apply in this order)
═══════════════════════════════════════════════════════════
1. NATURALNESS
   - Rewrite for native German speaker flow.
   - Restructure sentences if needed for idiomatic phrasing.
   - Prefer active constructions: "Die Forscher entdeckten" > "Es wurde entdeckt".
   - Use natural connectors: dabei, so, doch, allerdings, indes,
     statt: however → jedoch, thus → so/damit, therefore → daher.
   - Match register: neutral-informative documentary style.
     NO slang. NO bureaucratic language. NO academic jargon.

2. BREVITY  (target: 10–20 % shorter than input)
   - Remove redundant filler entirely:
     "Es ist wichtig zu erwähnen, dass" → delete entire phrase.
     "man kann sagen, dass" → delete.
   - Replace heavy noun constructions with verbs:
     "die Durchführung von Tests" → "Tests durchführen".
   - Use contracted forms in spoken registers:
     beim, im, zum, ins, ans, ums.
   - Prefer shorter synonyms:
     verwenden → nutzen | demonstrieren → zeigen
     durchführen → machen/tun | ermöglichen → erlauben/bieten
     aufweisen → haben/zeigen | bezeichnen als → nennen
     in Betracht ziehen → erwägen | in Frage stellen → bezweifeln

3. RHYTHM FOR TTS  (highest priority after meaning)
   - Avoid hard consonant clusters at phrase ends:
     BAD: "stärkt Städte" | GOOD: "macht Städte stärker"
   - Prefer sentences with clear prosodic breaks (comma positions).
   - Aim for balanced syllable distribution:
     no sentence should have 5+ consecutive stressed syllables.
   - Avoid compound nouns > 18 characters; break them:
     "Qualitätsmanagementsystem" → "System für Qualitätsmanagement"
   - Prefer 2–3 syllable words at sentence boundaries for smooth TTS.

4. REGISTER CONSISTENCY
   - Use "Sie"-form (formal) throughout — no sudden shifts to "du".
   - Keep tense consistent (usually Präsens for documentary narration).
   - Do NOT use archaic forms: anstatt dessen → stattdessen.

═══════════════════════════════════════════════════════════
ACCEPTABLE CONTRACTIONS  (spoken register)
═══════════════════════════════════════════════════════════
- in dem → im  |  an dem → am  |  zu dem → zum
- bei dem → beim  |  von dem → vom  |  zu der → zur
- in das → ins  |  auf das → aufs  |  an das → ans
- haben wir → "hab'n wir" ONLY if source is very informal, else keep formal

═══════════════════════════════════════════════════════════
THINGS TO NEVER CHANGE
═══════════════════════════════════════════════════════════
- Numbers, dates, proper names, acronyms (NATO, EU, DNA, etc.)
- Units of measurement
- Any term listed in the vocabulary hints below
- Sentence meaning or factual content

═══════════════════════════════════════════════════════════
FEW-SHOT POLISHING EXAMPLES
═══════════════════════════════════════════════════════════
INPUT: "Es ist wichtig zu erwähnen, dass die Forscher zu dem Schluss \
gekommen sind, dass die Daten eine erhebliche Auswirkung aufweisen."
OUTPUT: "Die Forscher schlussfolgern: Die Daten haben erhebliche Folgen."

INPUT: "Die Durchführung von umfangreichen Untersuchungen ermöglichte \
es dem Team, neue Erkenntnisse zu gewinnen."
OUTPUT: "Umfangreiche Untersuchungen brachten dem Team neue Erkenntnisse."

INPUT: "In dem Bereich der modernen Wissenschaft gibt es zahlreiche \
Entwicklungen, die in Betracht gezogen werden müssen."
OUTPUT: "In der modernen Wissenschaft sind viele Entwicklungen zu beachten."

INPUT: "Man kann sagen, dass dies eines der bedeutsamsten Ereignisse \
in der Geschichte der Menschheit war."
OUTPUT: "Dies war eines der bedeutsamsten Ereignisse der Menschheitsgeschichte."

═══════════════════════════════════════════════════════════
{vocabulary_support}
═══════════════════════════════════════════════════════════
OUTPUT FORMAT
- Return ONLY the polished German text.
- No labels, no quotes, no explanation, no prefix, no suffix.
...
"""
        ...
        for i in range(total_segments):
            ...
            if skip_threshold is not None and similarity >= skip_threshold:
            ...
            # ── Segment-spezifischer User-Prompt ──────────────
            user_prompt = (
                f"ENGLISH SOURCE (reference for meaning):\n{source_texts[i]}\n\n"
                f"CURRENT GERMAN (semantically correct, needs polishing):\n"
                f"{translated_texts[i]}\n\n"
                f"SEGMENT INFO:\n"
                f"  Quality score: {similarity:.3f}/1.0 "
                f"(lower = more room for improvement)\n"
                f"  Duration: {segment_duration:.1f}s | "
                f"Ideal max chars: {max_chars} | "
                f"Current chars: {original_len}\n\n"
                f"YOUR TASK:\n"
                f"Polish the German text above. Focus on:\n"
                f"  a) More natural word order and phrasing for a German narrator\n"
                f"  b) Remove any redundancy or filler words\n"
                f"  c) Shorten if possible, but ONLY if meaning is fully preserved\n"
                f"  d) Fix unnatural compound nouns (> 18 characters)\n"
                f"  e) Improve TTS rhythm (avoid hard consonant clusters)\n\n"
                f"Return only the improved German text."
            )
            ...
            try:
                ...
                polished = strip_llm_analysis(
                    response["message"]["content"].strip(),
                    translated_texts[i],
                )

                # ── Similarity des Polishing-Kandidaten messen ─
                # VRAM ~0.1 GB (einzelnes Encoding)
                with gpu_context():
                    model = model.to(device)
                    polished_embedding = model.encode(
                        polished, convert_to_tensor=True
                    )
                    polished_similarity: float = cos_sim(
                        embeddings_source[i : i + 1].to(device),
                        polished_embedding,
                    ).item()
                    model = model.to("cpu")
                    torch.cuda.empty_cache()

                # ── Semantische Verschlechterung verhindern ────
                original_similarity = similarities[i]
                if polished_similarity < original_similarity - 0.1:
                    tqdm.write(
                        f"  Segment {i}: ABGELEHNT – "
                        f"Semantische Verschlechterung "
                        f"({original_similarity:.3f} → {polished_similarity:.3f})"
                    )
                    polished_texts.append(translated_texts[i])
                    rejected_count += 1
                    polishing_results.append({
                        "index": i,
                        "status": "REJECTED",
                        "similarity_before": float(original_similarity),
                        "similarity_after": float(original_similarity),
                        "source_text": source_texts[i],
                        "text_before": translated_texts[i],
                        "text_after": translated_texts[i],
                    })
                ...
                best_similarity = max(polished_similarity, original_similarity)
                best_polished = (
                    polished
                    if polished_similarity >= original_similarity
                    else translated_texts[i]
                )

                polished_len = len(best_polished)
                tqdm.write(
                    f"  Segment {i}: "
                    f"{original_similarity:.3f} → {best_similarity:.3f} | "
                    f"{original_len} → {polished_len} Zeichen"
                )

                if best_polished != translated_texts[i]:
                    polished_count += 1
                    polished_similarities[i] = best_similarity
                    polishing_results.append({
                        "index": i,
                        "status": "✅ POLISHED",
                        "similarity_before": float(original_similarity),
                        "similarity_after": float(best_similarity),
                        "source_text": source_texts[i],
                        "text_before": translated_texts[i],
                        "text_after": best_polished,
                    })
                else:
                    unchanged_count += 1
                    polishing_results.append({
                ...
                logger.warning(
                    f"Polishing-Fehler bei Segment {i}: {e}", exc_info=True
                )
                polished_texts.append(translated_texts[i])
                unchanged_count += 1
        ...
        df_output["text"] = polished_texts
        df_output.to_csv(
            output_csv_path, sep="|", index=False, encoding="utf-8"
        )
        logger.info(f"Polierte CSV gespeichert: {output_csv_path}")

        avg_sim_before = avg_similarity_before
        avg_sim_after = (
            sum(polished_similarities) / len(polished_similarities)
            if polished_similarities
            else 0.0
        )
        improvement_pct = (
            (avg_sim_after - avg_sim_before) / avg_sim_before * 100
            if avg_sim_before > 0
            else 0.0
        )

        polished_segments = [
            r for r in polishing_results if r["status"] == "POLISHED"
        ]
        ...
        summary_content = (
            ...
            f"Datum der Prüfung:      {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            f"Modell (Similarity):    {model_name}\n"
            f"Modell (Polishing):     {llm_model_name}\n"
            f"Skip-Schwelle:          {skip_threshold}\n"
            f"Quell-Segmente:         {len(polishing_results)}\n"
            f"Übersprungen (≥ skip):  {skipped_count}\n"
            f"Poliert:                {polished_count}\n"
            f"Abgelehnt:              {rejected_count}\n"
            f"Unverändert:            {unchanged_count}\n"
            f"Ø Ähnlichkeit vorher:   {avg_sim_before:.4f}\n"
            f"Ø Ähnlichkeit nachher:  {avg_sim_after:.4f}\n"
            f"Verbesserung:           {improvement_pct:.1f}% "
            f"(absolut: {avg_sim_after - avg_sim_before:+.4f})\n"
            f"\n{'─' * 50}\n"
            f"Polierte Segmente:\n"
        )
        for item in polished_segments:
            summary_content += (
                f"\n  Segment {item['index']}: "
                f"{item['similarity_before']:.3f} → {item['similarity_after']:.3f}\n"
                f"  EN: {item['source_text']}\n"
                f"  DE vorher: {item['text_before']}\n"
                f"  DE nachher: {item['text_after']}\n"
            )
        ...
        print(
            f"\nSemantische Verbesserung: {improvement_pct:.1f}%"
        )
        print("-" * 43)
        ...
        logger.error(
            f"Eingabedatei nicht gefunden in polish_all_translations: {e}",
            exc_info=True,
        )
        raise
    except Exception as e:
        logger.error(
            f"Fehler in polish_all_translations: {e}", exc_info=True
        )
        raise
...
def refine_text_pipeline(
    ...
    try:
        with gpu_context():
            ...
            _special_tokens_to_add = [f"<extra_id_{i}>" for i in range(100)]
        ...
        # --- Schritt 1: Alle lokalen Modell-Referenzen explizit löschen ---
        # PunctuationModel hält BERT-Gewichte im VRAM (~450 MB)
        for var_name in ("punctuationmodel", "nlpmodel", "tokenizer"):
            if var_name in locals():
                try:
                    local_obj = locals()[var_name]
                    # Interne PyTorch-Module explizit leeren (thinc / transformers)
                    if hasattr(local_obj, "model"):
                        del local_obj.model
                    if hasattr(local_obj, "_model"):
                        del local_obj._model
                    del local_obj
                    logger.info(f"  del {var_name} erfolgreich.")
                except Exception as e:
                    logger.warning(f"  del {var_name} fehlgeschlagen (ignoriert): {e}")

        # --- Schritt 2: thinc-interne Gewichts-Buffer über spaCy leeren ---
        try:
            import spacy
            # Entfernt gecachte Modell-Klassen aus spaCys internem LRU
            if hasattr(spacy.util, "_data_path"):
                pass  # kein API zum Leeren, aber gc.collect() danach reicht
            # thinc Model-Registry leeren (hält Referenzen auf PyTorch Parameter)
            try:
                from thinc.api import get_current_ops
                ops = get_current_ops()
                if hasattr(ops, "xp") and hasattr(ops.xp, "get_default_memory_pool"):
                ...
                pass  # cupy nicht verfügbar → ignorieren
        except ImportError:
            pass

        # --- Schritt 3: Mehrfache GC-Runden (zirkuläre Referenzen auflösen) ---
        for _round in range(4):
            gc.collect()

        # --- Schritt 4: CUDA vollständig leeren ---
        if torch.cuda.is_available():
            torch.cuda.synchronize()
            torch.cuda.empty_cache()
            torch.cuda.ipc_collect()
            # Allocator-Statistiken zurücksetzen
            torch.cuda.reset_peak_memory_stats()
            torch.cuda.reset_accumulated_memory_stats()
            _alloc = torch.cuda.memory_allocated() / 1024**3
            _reserv = torch.cuda.memory_reserved() / 1024**3
            logger.info(
                f"  VRAM nach refine_text_pipeline: "
                f"Allocated={_alloc:.3f} GB | Reserved={_reserv:.3f} GB"
            )
...
# Synthetisieren
def _select_best_ref_audio(sample_paths: list[str]) -> str:
    """Wählt die längste gültige Referenz-Audiodatei aus einer Liste.

    Hintergrund: XTTS nutzte alle Samples simultan über
    get_conditioning_latents(). OmniVoice erwartet ein einziges ref_audio
    pro generate()-Aufruf. Strategie: Die längste Datei enthält die meiste
    Speaker-Information und eignet sich daher am besten für Voice Cloning.

    Args:
        sample_paths: Liste von WAV-Dateipfaden (z. B. sample_path_1..6).

    Returns:
        str: Pfad zur besten (längsten, gültigen) Referenz-Datei.

    Raises:
        FileNotFoundError: Wenn keine einzige gültige Datei gefunden wird.
    """
    valid_samples: list[tuple[float, str]] = []

    for path in sample_paths:
        # Leere Einträge und fehlende Dateien überspringen
        if not path or not os.path.exists(path):
            logger.debug(f"Referenz-Audio übersprungen (nicht gefunden): {path}")
            continue
        try:
            info = sf.info(path)  # soundfile.SoundFileInfo
            valid_samples.append((info.duration, path))
            logger.debug(
                f"Gültige Referenz: {Path(path).name} ({info.duration:.1f} s)"
            )
        except Exception as exc:
            logger.warning(f"Konnte Sample '{path}' nicht lesen: {exc}")

    if not valid_samples:
        raise FileNotFoundError(
            f"Keine gültige Referenz-Audio-Datei in der Liste: {sample_paths}"
        )

    # Längste Datei maximiert die Speaker-Information für OmniVoice
    best_duration, best_path = max(valid_samples, key=lambda item: item[0])
    logger.info(
        f"Bestes Referenz-Audio gewählt: "
        f"'{Path(best_path).name}' ({best_duration:.1f} s) "
        f"aus {len(valid_samples)} gültigen Samples"
    )
    return best_path
...
def text_to_speech_with_voice_cloning_omnivoice(
    ...
    """Synthetisiert übersetzte Segmente mit OmniVoice Voice Cloning.

    Identische Außenschnittstelle wie die frühere XTTS-Version.
    Alle anderen Funktionen (load_existing_progress, load_segments_from_file,
    filter_new_segments, log_progress, assemble_final_audio_ffmpeg_sequential)
    bleiben unverändert.

    Gegenüber XTTS geändert:
    - load_xtts_v2()             → load_omnivoice()
    - get_conditioning_latents() → entfällt; ref_audio direkt an generate()
    - model.inference(...)       → model.generate(...)
    - result["wav"]              → result[0]  (list[np.ndarray])
    - Explizites del + cleanup   im finally-Block (VRAM freigeben)

    Args:
        translation_file: Pfad zur CSV-Datei mit den übersetzten Segmenten.
        sample_path_1..6: Pfade zu Referenz-Audios für Voice Cloning.
        output_path: Zielpfad der finalen Audiodatei.
        speaker_name: Optionaler Name (nur Logging, nicht funktional).
    """
    stretched_chunks_dir: str = os.path.join(TTS_TEMP_CHUNKS_DIR, "stretched")

    # ── SCHRITT 1: Segmente laden und filtern ─────────────────────────────────
    processed: set = load_existing_progress()
    all_segments: list = load_segments_from_file(translation_file)
    ...
    if not segments_to_process:
        ...
        if os.path.exists(output_path):
            logger.info(
                f"Finale TTS-Audiodatei '{output_path}' existiert bereits "
                "und es gibt nichts Neues zu tun."
            )
        return  # Funktion sicher beenden

    logger.info(f"{len(segments_to_process)} neue Segmente werden synthetisiert")

    # ── SCHRITT 2: Ausgabedatei-Überprüfung ───────────────────────────────────
    if os.path.exists(output_path):
        if not ask_overwrite(output_path):
            logger.info(
                f"Benutzer hat das Überschreiben von '{output_path}' abgelehnt. "
                "TTS-Synthese wird abgebrochen."
            )
            return
        else:
            logger.info(
                f"Benutzer hat dem Überschreiben von '{output_path}' zugestimmt. "
                "Alte temporäre Dateien werden entfernt."
            )
    ...
    # Temporäre Verzeichnisse anlegen
    os.makedirs(TTS_TEMP_CHUNKS_DIR, exist_ok=True)
    os.makedirs(stretched_chunks_dir, exist_ok=True)

    # ── SCHRITT 3: Bestes Referenz-Audio wählen ───────────────────────────────
    # OmniVoice akzeptiert 1 ref_audio pro Aufruf (kein batch conditioning).
    # _select_best_ref_audio() wählt die längste gültige Datei.
    ...
    ref_audio_path: str = _select_best_ref_audio(sample_paths)
    ...
    with gpu_context():
        omnivoice_model: Optional[OmniVoice] = None
        try:
            print("------------------")
            print("|<< Starte TTS >>|")
            print("------------------")
            ...
            for segment_info in tqdm(
                ...
                try:
                    with torch.inference_mode():
                        # VRAM ~0.5–1 GB zusätzlich für Aktivierungen während Inferenz
                        # OmniVoice.generate() → list[np.ndarray], 24 kHz
                        result: list = omnivoice_model.generate(
                            text=segment_info["text"],
                    ...
                    # OmniVoice gibt list[np.ndarray] zurück (XTTS: result["wav"])
                    if not result or len(result) == 0:
                        logger.warning(
                            f"Leeres Ergebnis für Segment {segment_info['id']}."
                        )
                        continue

                    audio_clip: np.ndarray = result[0]
                    ...
                    # Audio als WAV speichern – 24000 Hz identisch zu XTTS
                    chunk_filename: str = f"chunk_{segment_info['id']}.wav"
                    chunk_path: str = os.path.join(
                        TTS_TEMP_CHUNKS_DIR, chunk_filename
                    )
                    sf.write(chunk_path, audio_clip, OMNIVOICE_SAMPLE_RATE)
                    log_progress(segment_info, chunk_path)

                except Exception as seg_err:
                    logger.error(
                        f"Fehler bei der TTS-Synthese für Segment "
                        f"{segment_info['id']}: {seg_err}",
                        exc_info=True,
                    )
                    continue  # Zum nächsten Segment springen

            # ── SCHRITT 5: Audio-Assemblierung (unverändert) ──────────────────
            assemble_final_audio_ffmpeg_sequential(output_path)

            logger.info(
                f"TTS-Prozess abgeschlossen. Finale Audiodatei: {output_path}"
            )
            print("---------------------------")
            ...
            print(f"{tts_end_time / 3600:.2f} Stunden")
            logger.info(
                f"TTS-Audio mit geklonter Stimme erstellt: {output_path}"
            )
            logger.info("🎉 OmniVoice Voice-Cloning-Synthese abgeschlossen")

        except Exception as fatal_err:
            logger.critical(
                "Ein schwerwiegender Fehler ist im TTS-Prozess aufgetreten: "
                f"{fatal_err}",
                exc_info=True,
            )

        finally:
            # Modell explizit aus VRAM entfernen (verhindert VRAM-Leak)
            if omnivoice_model is not None:
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        torch.cuda.synchronize()
                    logger.info(
                        "OmniVoice-Modell erfolgreich aus VRAM entfernt."
                    )
                except Exception as cleanup_err:
                    logger.warning(
                        f"Fehler beim OmniVoice-VRAM-Cleanup: {cleanup_err}"
                    )
...
def main():
    ...
    try:
        ...
        if EXECUTE_TRANSCRIPTION:
            ...
            clear_spacy_cache_and_free_vram()
            purge_all_vram_before_translation()
            time.sleep(3)
        ...
        if EXECUTE_TRANSLATION:
            ...
            select_best_hypotheses_post_translation(
                ...
                similarity_model_name=ST_BETTER_MODEL
            )
