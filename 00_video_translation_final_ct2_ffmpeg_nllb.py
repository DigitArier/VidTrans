...
def load_nllb200_translator_ct2(model_dir: str = NLLB_MODEL_DIR,
    ...
    compute_type = "int8_bfloat16" if device == "cuda" else "int8"
...
def translate_batch_nllb200(
    ...
    """
    Übersetzt eine Liste bereits *unveränderter* Quelltexte per NLLB-200.
    Jeder Aufruf behält Personen-/Platzhalter u. Ä. exakt bei.
    """

    if not texts:
        return []

    # 1  Tokenisierung inkl. Source-Sprachcode
    ...
    # in Token-Strings für CTranslate2 umwandeln
    tokens_src = [tokenizer.convert_ids_to_tokens(ids) for ids in encoded]

    # 2  Ziel-Prefix  (<2xx_Latn> Token als separate Liste)
    target_prefix = [[f"<2{tgt_lang}>"]] * len(texts)
    ...
    results = translator.translate_batch(
        ...
        beam_size=beam_size,
        max_batch_size=NLLB_BATCH_MAX_TOKENS,
        batch_type="tokens",
        no_repeat_ngram_size=2,
        repetition_penalty=1.2
    )

    # 4  Dekodieren & <2xx>-Prefix entfernen
    out_tokens = []
    for res in results:
        toks = res.hypotheses[0]
        if toks and toks[0].startswith("<2"):
            toks = toks[1:]
        out_tokens.append(toks)

    translations = tokenizer.batch_decode(
        [tokenizer.convert_tokens_to_ids(t) for t in out_tokens],
...
def translate_batch_ollama(  
    texts: List[str],  
    model: str = "D:\\Modelle\\Meta-Llama-3.1-8B-Instruct.Q4_K_M.gguf",  # Dein Modell  
    target_lang: str = "de",  
    ...
    translations = []  
    for text in texts:  
        prompt = f"Translate to  {target_lang}: {text}. Keep placeholders like <extra_id_#> unchanged."  
...
def translate_segments_optimized_safe(
    ...
    with gpu_context():
        ...
        #translator, tokenizer = load_madlad400_translator_ct2()
        ...
        for i in tqdm(range(0, len(segments_to_process), batch_size), desc="Übersetze geschützte Batches"):
            ...
            #translated_protected_texts = translate_batch_llama_gguf(
                #texts=texts_to_translate, 
                #gguf_model_path="D:\\Modelle\\Meta-Llama-3.1-8B-Instruct.Q4_K_M.gguf", 
