...
def translate_batch_madlad(
    ...
    results = translator.translate_batch(
        ...
        batch_type="tokens",
        max_batch_size=1024,
        beam_size=6,              # Fest auf 5; dynamischer Wert war ineffizient
        num_hypotheses=num_hypotheses,
        patience=1.0,             # War 1.0 → tiefere Suche, bessere Kandidaten
        length_penalty=0.8,       # War 1.0 → kompaktere, natürlichere Ausgaben
        repetition_penalty=1.0,   # War 1.2 → hoher Wert kürzte Sätze ab
    ...
    try:
        ...
        for i in range(len(df_source)):
            ...
            if similarity < threshold:
                ...
                _CLIFFHANGER_ENDINGS: tuple[str, ...] = (
                    "dass.", "weil.", "und.", "oder.", "aber.", "denn.",
                    "der.", "die.", "das.", "den.", "dem.",
                    "that.", "which.", "who.", "but.", "and.", "because.",
                    "wenn.", "als.", "obwohl.", "damit.", "sodass.",
                )
                _de_tail = translated_texts[i].strip().lower()
                _en_tail = source_texts[i].strip().lower()
                _is_cliffhanger = (
                    any(_de_tail.endswith(end) for end in _CLIFFHANGER_ENDINGS)
                    or any(_en_tail.endswith(end) for end in _CLIFFHANGER_ENDINGS)
                )
                if _is_cliffhanger:
                    tqdm.write(
                        f"   ⏭ Segment {i} ist ein Cliffhanger-Satz "
                        f"(endet mit '{translated_texts[i].strip().split()[-1]}') — "
                        f"Korrektur übersprungen, Original beibehalten."
                    )
                    final_corrected_text = translated_texts[i]
                    correction_note = "Cliffhanger-Segment: absichtlich unvollständig, keine Korrektur möglich."
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        system_prompt = f"""You are a senior German language expert and professional subtitler specializing \
                                                in voice-over localization (Osteuropa-Stil: single narrator, neutral register).

                                                ## YOUR ROLE
                                                You receive an English subtitle segment and a flawed German translation.
                                                Your task: Produce the single best corrected German translation.

                                                ## PRIORITY ORDER (highest to lowest)
                                                1. SEMANTIC FIDELITY — The German MUST convey the exact same meaning as the English source.
                                                - Never add, remove, or alter information.
                                                - Preserve the grammatical mood: indicative stays indicative, questions stay questions.
                                                - Historical present in English → historical present in German (Er geht, nicht: Er ging).
                                                2. NATURAL GERMAN GRAMMAR
                                                - V2 word order in main clauses (Verb at position 2).
                                                - Verb-final in subordinate clauses (…, weil er es wusste.).
                                                - Active voice preferred over passive.
                                                - No Anglicisms unless the original uses them as proper nouns.
                                                3. CONCISENESS FOR TTS/SUBTITLES
                                                - Target length: ≤ {max_chars} characters (hard limit).
                                                - Prefer shorter synonyms: "nutzen" over "verwenden", "zeigen" over "demonstrieren".
                                                - Split one long German sentence into two short ones if clarity improves.
                                                - Avoid compound nouns > 18 characters; break them: "Sicherheitsprotokoll" → ok, "Qualitätsmanagementsystem" → "System für Qualitätsmanagement".
                                                4. SPOKEN RHYTHM
                                                - The text will be read aloud by a TTS voice. Avoid dental clusters and hard consonant chains.
                                                - Prefer flowing syllable patterns over staccato noun stacks.

                                                ## COMMON ERROR PATTERNS TO FIX
                                                - "declare that they have" → INDICATIVE: "erklären, dass sie … haben" (NOT Konjunktiv II)
                                                - English gerund "by doing X" → German "indem sie X tun" (not "durch das Tun von X")
                                                - "there is / there are" → "Es gibt" or restructure entirely ("Drei Gruppen existieren.")
                                                - False friends: "eventually" → "schließlich" (NOT "eventuell"), "actual" → "tatsächlich" (NOT "aktuell")
                                                - Overlong compounds: always break them up as shown above.
                                                - Pleonasms: "in der Vergangenheit früher" → "früher"

                                                ## VOCABULARY HINTS (authoritative for this project)
                                                {vocabulary_support}
                                                ...
                                                # Output EXACTLY one block of German text. Nothing else.
                                                # No "Error Analysis", no numbered lists, no bold text, no asterisks,
                                                # no explanations before or after the translation.
                                                # If you write ANYTHING other than the German translation, your output is invalid."""
                        ...
                        user_prompt = (
                            f"SOURCE (EN) — translate the ENTIRE block below, every sentence:\n"
                            f"<<<\n{source_texts[i]}\n>>>\n\n"
                            f"CURRENT GERMAN — improve the ENTIRE block, not just the last sentence:\n"
                            f"<<<\n{translated_texts[i]}\n>>>\n\n"
                            f"Duration: {segment_duration:.2f}s | Hard limit: {max_chars} chars\n\n"
                            f"OUTPUT: The complete corrected German text for ALL sentences above. "
                            f"No analysis, no labels, no markdown. German text only. Max {max_chars} characters."
                        )
                        ...
                        def _strip_llm_analysis(raw: str, source_de: str) -> str:
                            """Extrahiert die reine Übersetzung aus einem ggf. Markdown-angereicherten LLM-Output.

                            Strategie:
                            1. Wenn kein Markdown-Marker (*,#,**) → direkt zurückgeben.
                            2. Sonst: suche die erste Zeile, die kein Markdown ist und > 20 Zeichen hat.
                            3. Fallback: original source_de zurückgeben.
                            """
                            if not any(marker in raw for marker in ("**", "##", "---", "1.", "Error")):
                                return raw  # Kein Analyse-Müll erkennbar

                            # Suche nach Zeilen ohne Markdown-Marker, die wie eine Übersetzung aussehen
                            lines = raw.splitlines()
                            candidates = [
                                ln.strip("*_`> ")
                                for ln in lines
                                if len(ln.strip()) > 20
                                and not ln.strip().startswith(("#", "*", "-", "1", "2", "3", "4", "5", "|"))
                                and "**" not in ln
                                and "Error" not in ln
                            ...
                            if candidates:
                                # Nehme den längsten Kandidaten (wahrscheinlich die eigentliche Übersetzung)
                                return max(candidates, key=len)

                            return source_de  # Sicherer Fallback: Original behalten


                        correction_candidate = _strip_llm_analysis(
                            response['message']['content'].strip(),
                            translated_texts[i],   # Fallback = bisherige DE-Übersetzung
                        )
...
def polish_all_translations(
    ...
    for i in range(total_segments):
        ...
        polishing_system_prompt = f"""You are a native German language editor specializing in spoken-word \
                                        localization for documentary and factual content (voice-over, single narrator style).

                                        ## YOUR ROLE
                                        You receive a semantically correct German subtitle segment and must make it more natural, \
                                        concise, and pleasant to hear — without changing the meaning.

                                        ## EDITING PRIORITIES (apply in this order)
                                        1. NATURALNESS — Rewrite for native German speaker flow.
                                        - Restructure sentences if needed for idiomatic phrasing.
                                        - Prefer active constructions over passive ("Die Forscher entdeckten" > "Es wurde entdeckt").
                                        - Use natural connectors: "dabei", "so", "doch", "allerdings" instead of literal "however", "thus".
                                        - Match register: neutral-informative (no slang, no overly formal bureaucratic language).
                                        2. BREVITY — Target 10–20% shorter than the input, without information loss.
                                        - Remove redundant filler: "Es ist wichtig zu erwähnen, dass" → delete entirely.
                                        - Replace heavy noun constructions with verbs: "die Durchführung von Tests" → "Tests durchführen".
                                        - Use contracted forms in spoken registers: "beim", "im", "zum", "ins".
                                        3. RHYTHM FOR TTS
                                        - Avoid consonant clusters at phrase ends (e.g., "stärkt Städte" is hard to render).
                                        - Prefer sentences with clear prosodic breaks (comma positions).
                                        - Aim for balanced syllable distribution — no sentence should have 5+ consecutive stressed syllables.

                                        ## STYLE REFERENCE: ACCEPTABLE CONTRACTIONS
                                        - "haben wir" → "hab'n wir" only if very informal source; otherwise keep formal.
                                        - "in dem" → always "im", "an dem" → always "am", "zu dem" → always "zum".
                                        - "es ist" → "es ist" (no contraction), but "das ist" → "das ist" (keep).
        ...
        user_prompt = f"""## ENGLISH SOURCE (reference for meaning)
                            {source_texts[i]}

                            ## CURRENT GERMAN (semantically correct, needs polishing)
                            {translated_texts[i]}

                            ## SEGMENT INFO
                            - Quality score: {similarity:.3f} / 1.0  (lower = more room for improvement)
                            - Duration: {segment_duration:.1f}s | Ideal max chars: {max_chars} | Current chars: {len(translated_texts[i])}

                            ## YOUR TASK
                            Polish the German text above. Focus on:
                            a) More natural word order and phrasing for a German narrator
                            b) Remove any redundancy or filler words
                            c) Shorten if possible — but ONLY if meaning is fully preserved
                            d) Fix any unnatural compound nouns > 18 characters

                            Return only the improved German text."""
        ...
        try:
            for attempt, temp in enumerate(temperatures, start=1):
                ...
                response = ollama.chat(
                    model="translategemma:4b",   # oder "qwen3:8b" als bewährte Wahl
                    messages=[
                    ...
                    options={
                        "temperature":  temp,
                        "num_ctx":      2048,          # TranslateGemma hat nur 2K Kontext-Limit
                        "num_gpu":      33,            # RTX 4050: alle Layer auf GPU
                        "num_thread":   6,
                        "main_gpu":     0,
                        "use_mmap":     True,
                        "use_mlock":    False,
