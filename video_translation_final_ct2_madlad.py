...
MAX_GAP = 2 # Maximaler akzeptierter Zeitabstand zwischen Segmenten
...
def load_translation_model(model_path=None):
    ...
    translator = ctranslate2.Translator(
        ...
        inter_threads=8,  # Parallelisierung für mehrere Eingaben
...
def load_xtts_v2():
    ...
    config = XttsConfig()
    config.load_json(r"D:\alltalk_tts\models\xtts\v202\config.json")
    # 2) Modell initialisieren
    xtts_model = Xtts.init_from_config(config)
    xtts_model.load_checkpoint(
        config,
        checkpoint_dir=r"D:\alltalk_tts\models\xtts\v202",  # Pfad anpassen
        use_deepspeed=False
    )
...
def batch_translate(batch_texts, translator, tokenizer, target_lang):
    ...
    try:
        ...
        for item in batch_texts:
            ...
            prefixed_text = f"<2{target_lang}> {text}"
        ...
        results = translator.translate_batch(
            ...
            beam_size=5,          # Anpassen nach Qualität/Geschwindigkeit
...
#   6.1) Zusammenführen von Transkript-Segmenten
    ...
    text_to_speech_with_voice_cloning(TTS_FORMATTED_TRANSLATION_FILE,
                                    SAMPLE_PATH_1,
                                    SAMPLE_PATH_2,
                                    #SAMPLE_PATH_3,
                                    #SAMPLE_PATH_4,
